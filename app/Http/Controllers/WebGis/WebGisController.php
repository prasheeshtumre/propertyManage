...
use App\Models\{UnitAmenity, UnitAmenityOption, Budget, AreaMeasurement, FloorType, Parcel, Property, Ammenitity, Category, FloorUnitCategory, FloorUnitMap, PropertyFloorMap, SecondaryUnitLevelData, User};
use Carbon\Carbon;
use DB;
use Auth;
use PSpell\Config;
...
class WebGisController extends Controller
{
    ...
    {
        $surveyors = User::with('roles')
            ->whereHas('roles', function ($query) {
                $query->whereIn('slug', ['surveyor']);
            })
            ->get();
    ...
    // public function testSearchedFeature(Request $request) {
    //     $properties = Property::select('properties.*', DB::raw('ST_AsText(geom) AS geometry'))->leftJoin('Parcel', 'properties.gis_id', '=', 'Parcel.uniq_id')->where('created_by', Auth::user()->id)->get();
    //     echo($properties);
    // }
