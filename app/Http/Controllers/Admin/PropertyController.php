...
use App\Models\{AreaMeasurement, Category, SubCategory, PropertyImage, Property, PropertyFloorMap, FloorUnitMap, FloorUnitCategory, Builder, Tower, GisIDMapping, GeoID,ConstructionPartner};
...
class PropertyController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.pages.property.demo_index', compact('categories', 'sub_categories'));
    }
    ...
    public function ajaxReports(Request $request, $type = null)
    {
        ...
        $subCategories = Category::where('parent_id', 1)
            ->OrderBy('id', 'ASC')
            ->get();
        $constructionPartner = ConstructionPartner::OrderBy('id', 'ASC')
        ->get();
        ...
        // }
        $properties->setPath(route('admin.property.reports', [], true));
        ...
        if ($request->ajax()) {
            if ($request->category == 1 || $request->category == 6) {
                return view('admin.pages.property.property_pagination', ['properties' => $properties, 'category_type' => $request->category, 'property_count' => $property_count, 'subCategories' => $request->subCategories]);
    ...
    {
        return Excel::download(new PropertiesExport(), 'properties.csv', \Maatwebsite\Excel\Excel::CSV);
    }
